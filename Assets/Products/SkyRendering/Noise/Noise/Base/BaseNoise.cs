...
public class BaseNoise : MonoBehaviour {
    ...
    public bool autoRealtimePreview;
    ...
    public virtual void Generate() {
        ...
        int coreKernel = cs_core.FindKernel(GetCoreKernelName());
        cs_core.SetBuffer(coreKernel, "_Colors", tempComputeBuffer);
        cs_core.SetTexture(coreKernel, "_Texture2D", tempRenderTexture2D);
        cs_core.SetTexture(coreKernel, "_Texture3D", tempRenderTexture3D);
        ...
        cs_core.Dispatch(coreKernel, dispatchX, dispatchY, resolutionZ);

        if (ShouldPostProcess()) {
            int postKernel = cs_postProcess.FindKernel("Main");
            cs_postProcess.SetBuffer(postKernel, "_Colors", tempComputeBuffer);
            cs_postProcess.SetTexture(postKernel, "_Texture2D", tempRenderTexture2D);
            cs_postProcess.SetTexture(postKernel, "_Texture3D", tempRenderTexture3D);
            ...
            cs_postProcess.Dispatch(postKernel, dispatchX, dispatchY, resolutionZ);
        }
    ...
    public bool HasGeneratedData() {
        return tempComputeBuffer != null;
    }

    public Texture2D GetPreviewTexture2D(int zSlice) {
        if (tempComputeBuffer == null || resolution <= 0) {
            return null;
        }

        int pixelCount = resolution * resolution;
        int depth = is3D ? resolution : 1;
        int clampedSlice = Mathf.Clamp(zSlice, 0, depth - 1);
        int sliceOffset = clampedSlice * pixelCount;

        Color[] allColors = new Color[tempComputeBuffer.count];
        tempComputeBuffer.GetData(allColors);

        Color[] sliceColors = new Color[pixelCount];
        for (int i = 0; i < pixelCount; i++) {
             sliceColors[i] = allColors[sliceOffset + i];
            // float value = Mathf.Clamp01(allColors[sliceOffset + i].r);
            // sliceColors[i] = new Color(value, value, value, 1f);
        }

        if (tempPreviewTexture == null || tempPreviewTexture.width != resolution || tempPreviewTexture.height != resolution) {
            if (tempPreviewTexture != null) {
                DestroyImmediate(tempPreviewTexture);
            }

            tempPreviewTexture = new Texture2D(resolution, resolution, TextureFormat.R8, false);
            tempPreviewTexture.wrapMode = TextureWrapMode.Repeat;
            tempPreviewTexture.filterMode = FilterMode.Bilinear;
        }

        tempPreviewTexture.SetPixels(sliceColors);
        tempPreviewTexture.Apply(false, false);
        return tempPreviewTexture;
    }
    ...
    protected virtual string GetCoreKernelName() {
        return "Main";
    }
