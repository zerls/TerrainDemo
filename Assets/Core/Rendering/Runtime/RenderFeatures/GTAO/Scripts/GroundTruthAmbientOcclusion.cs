    ...
    internal class GroundTruthAmbientOcclusion : ScriptableRendererFeature
    {
        ...
        private class ScreenSpaceAmbientOcclusionPass : ScriptableRenderPass
        {
            ...
            private RenderTargetIdentifier m_SSAOTextureFinalTarget =
                new RenderTargetIdentifier(s_SSAOTextureFinalID, 0, CubemapFace.Unknown, -1);
            ...
            private static readonly int s_SSAOTextureFinalID = Shader.PropertyToID("_ScreenSpaceOcclusionTexture");
            ...
            public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
            {
                ...
                Vector2 focalLen =
                    new Vector2(
                        invHalfTanFov * (((float)renderingData.cameraData.camera.pixelHeight / (float)downsampleDivider) /
                                         ((float)renderingData.cameraData.camera.pixelWidth / (float)downsampleDivider)), invHalfTanFov);
                ...
                ConfigureTarget(m_CurrentSettings.AfterOpaque ? m_Renderer.cameraColorTarget : s_SSAOTextureFinalID);
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                using (new ProfilingScope(cmd, m_ProfilingSampler))
                {
                    ...
                    if (!m_CurrentSettings.Debug)
                    {
                        cmd.SetGlobalVector(k_SSAOAmbientOcclusionParamName, new Vector4(1f, 0f, 0f, m_CurrentSettings.DirectLightingStrength));
                    }
                    else
                    {
            ...
            private void RenderAndSetBaseMap(CommandBuffer cmd, RenderTargetIdentifier baseMap, RenderTargetIdentifier target,
                ShaderPasses pass)
            {
